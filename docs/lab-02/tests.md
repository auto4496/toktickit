...
| UNIT-04 | Unit | BR-27-BR-30, BR-35, AC-14-AC-16, AC-18 | Attachment type, size, count, filename, and reason rules | Exact boundaries and approved pairs pass; invalid cases fail safely | `server/tests/lab-02/attachment-validation.unit.test.ts` | Planned |
| API-01 | API | FR-02, FR-07, BR-02, AC-01, AC-25 | Active Requesters, Categories, and Related Systems | Only active rows returned in documented order and shapes | `server/tests/lab-02/reference-data.api.test.ts` | Planned |
...
| API-05 | API | FR-11, BR-17-BR-18, AC-06 | Ticket idempotency | Same key/payload replays original; changed payload returns `409`; no duplicate row | `server/tests/lab-02/create-ticket.api.test.ts` | Planned |
...
| API-07 | API | FR-14, BR-04-BR-07, AC-08 | My Tickets ownership and pagination metadata | Only selected Requester's Tickets returned with accurate metadata | `server/tests/lab-02/my-tickets.api.test.ts` | Planned |
| API-08 | API | FR-15-FR-16, BR-20-BR-24, AC-09 | Search, each filter, sort, secondary order, and page boundaries | Deterministic documented subset and order | `server/tests/lab-02/my-tickets.api.test.ts` | Planned |
...
| API-10 | API | FR-19, FR-21, AC-11 | Owned Ticket Detail | `200` read-only data shape with owned Attachment metadata | `server/tests/lab-02/ticket-detail.api.test.ts` | Planned |
...
| API-12 | API | FR-22-FR-24, BR-27-BR-33, AC-13 | Valid Attachment upload and metadata | `201`; safe metadata saved after file storage; private path absent | `server/tests/lab-02/attachments.api.test.ts` | Planned |
| API-13 | API | FR-23, BR-27-BR-28, AC-14-AC-15 | Unsupported, mismatched, and oversized files | `415` or `413`; no metadata/file residue | `server/tests/lab-02/attachments.api.test.ts` | Planned |
| API-14 | API | BR-29, AC-16 | Five-active Attachment boundary | Sixth rejected `409`; replacement accepted after soft removal | `server/tests/lab-02/attachments.api.test.ts` | Planned |
| API-15 | API | FR-24-FR-25, BR-31-BR-32, AC-17 | Active metadata and download | Correct metadata/headers/bytes; storage details absent | `server/tests/lab-02/attachments.api.test.ts` | Planned |
| API-16 | API | FR-26-FR-27, BR-35-BR-37, AC-18-AC-19 | Soft removal and removed access | Metadata retained; valid reason saved; download returns safe `404` | `server/tests/lab-02/attachments.api.test.ts` | Planned |
...
| UI-01 | UI | FR-01-FR-06, AC-01-AC-03 | Requester selector states, route guard, selection, and Change Requester | Required text/states render; Continue works; switch clears old data | `client/tests/lab-02/RequesterSelection.test.tsx` | Planned |
...
| UI-03 | UI | FR-08-FR-09, AC-05 | Create Ticket field and Attachment-selection validation | Inline linked errors; API not called for invalid form/files | `client/tests/lab-02/CreateTicket.test.tsx` | Planned |
| UI-04 | UI | FR-10-FR-12, AC-04, AC-06 | Create busy and success states | Submit disabled while pending; official number and backend values shown once | `client/tests/lab-02/CreateTicket.test.tsx` | Planned |
...
| UI-07 | UI | FR-14-FR-18, AC-08-AC-10 | My Tickets loaded, loading, empty, no-results, and failure states | Correct distinct states, results, and accessible actions | `client/tests/lab-02/MyTickets.test.tsx` | Planned |
...
| UI-09 | UI | FR-19-FR-21, AC-11-AC-12 | Ticket Detail loading, owned data, and not-found state | Read-only information and separate Attachment section; safe not-found | `client/tests/lab-02/RequesterTicketDetail.test.tsx` | Planned |
| UI-10 | UI | FR-22-FR-27, AC-13-AC-19 | Attachment active, uploading, invalid, failed, and removed states | Correct controls, reason dialog, retained metadata, and disabled removed access | `client/tests/lab-02/AttachmentSection.test.tsx` | Planned |
