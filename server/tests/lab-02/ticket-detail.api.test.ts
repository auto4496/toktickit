...
beforeAll(async () => {
  ...
  await prisma.user.createMany({ data: [
    { id: requesterAId, name: 'Detail Requester A', email: 'detail.a@example.test', isActive: true, ...fixtureCredential },
    { id: requesterBId, name: 'Detail Requester B', email: 'detail.b@example.test', isActive: true, ...fixtureCredential },
  ] });
  const category = await prisma.category.findFirstOrThrow({ where: { isActive: true } });
  const relatedSystem = await prisma.relatedSystem.findFirstOrThrow({ where: { isActive: true } });
  const ticket = await prisma.ticket.create({ data: { ticketNumber: 'TKT-20260901-16000001', requesterId: requesterAId, categoryId: category.id, relatedSystemId: relatedSystem.id, summary: 'Owned detail Ticket', description: 'An owned detail description for Issue 16.', requestedPriority: 'MEDIUM', itPriority: 'MEDIUM' } });
  ticketId = ticket.id;
});
...
describe('GET /api/tickets/:ticketId', () => {
  it('returns one owned, read-only detail shape without private storage fields', async () => {
    const response = await request(app).get(`/api/tickets/${ticketId}`).set(await sessionHeaders(requesterAId));
  ...
  it('returns the identical safe 404 for a missing or another Requester ticket', async () => {
    const headers = await sessionHeaders(requesterBId);
