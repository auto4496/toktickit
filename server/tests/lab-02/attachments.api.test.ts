...
beforeAll(async () => {
  ...
  await prisma.user.createMany({ data: [
    { id: requesterAId, name: 'Attachment Requester A', email: 'attachment.a@example.test', isActive: true, ...fixtureCredential },
    { id: requesterBId, name: 'Attachment Requester B', email: 'attachment.b@example.test', isActive: true, ...fixtureCredential },
  ] });
  const category = await prisma.category.findFirstOrThrow({ where: { isActive: true } });
  const relatedSystem = await prisma.relatedSystem.findFirstOrThrow({ where: { isActive: true } });
  const ticket = await prisma.ticket.create({ data: { ticketNumber: 'TKT-20260901-16000002', requesterId: requesterAId, categoryId: category.id, relatedSystemId: relatedSystem.id, summary: 'Attachment lifecycle Ticket', description: 'Ticket used to verify Attachment lifecycle.', requestedPriority: 'LOW', itPriority: 'LOW' } });
  ticketId = ticket.id;
  const concurrentTicket = await prisma.ticket.create({ data: { ticketNumber: 'TKT-20260901-16000003', requesterId: requesterAId, categoryId: category.id, relatedSystemId: relatedSystem.id, summary: 'Concurrent Attachment Ticket', description: 'Ticket used to verify concurrent Attachment admission.', requestedPriority: 'LOW', itPriority: 'LOW' } });
...
describe('Attachment lifecycle', () => {
  it('uploads verified bytes, returns safe metadata, downloads, and retains soft-removal metadata', async () => {
    const upload = await request(app).post(`/api/tickets/${ticketId}/attachments`).set(await sessionHeaders(requesterAId)).attach('file', png, { filename: 'vpn-error.PNG', contentType: 'image/png' });
    ...
    const download = await request(app).get(`/api/attachments/${attachmentId}/download`).set(await sessionHeaders(requesterAId));
    expect(download.status).toBe(200);
    expect(download.headers['x-content-type-options']).toBe('nosniff');
    expect(Buffer.from(download.body)).toEqual(png);
    const removed = await request(app).delete(`/api/attachments/${attachmentId}`).set(await sessionHeaders(requesterAId)).send({ reason: 'Uploaded the wrong screenshot.' });
    ...
    const removedDownload = await request(app).get(`/api/attachments/${attachmentId}/download`).set(await sessionHeaders(requesterAId));
    expect(removedDownload.status).toBe(404);
  });

  it('does not disclose Attachment metadata to a different Requester', async () => {
    const response = await request(app).get(`/api/attachments/${attachmentId}`).set(await sessionHeaders(requesterBId));
  ...
  it('checks ownership before parsing an uploaded file and uses documented upload error codes', async () => {
    ...
    const invalidId = await request(app).post('/api/tickets/not-a-uuid/attachments').set(await sessionHeaders(requesterAId)).attach('file', png, { filename: 'bad-id.png', contentType: 'image/png' });
    expect(invalidId.status).toBe(400);
    expect(invalidId.body.error.code).toBe('INVALID_TICKET_ID');
    const nonOwned = await request(app).post(`/api/tickets/${ticketId}/attachments`).set(await sessionHeaders(requesterBId)).attach('file', Buffer.from('not png'), { filename: 'bad.png', contentType: 'image/png' });
    expect(nonOwned.status).toBe(404);
    expect(nonOwned.body.error.code).toBe('RESOURCE_NOT_FOUND');
    const missing = await request(app).post(`/api/tickets/${ticketId}/attachments`).set(await sessionHeaders(requesterAId));
    expect(missing.status).toBe(400);
    expect(missing.body.error.code).toBe('FILE_REQUIRED');
    const unsupported = await request(app).post(`/api/tickets/${ticketId}/attachments`).set(await sessionHeaders(requesterAId)).attach('file', Buffer.from('bad'), { filename: 'bad.png', contentType: 'image/png' });
  ...
  it('covers filename, extension, MIME, signature, and exact HTTP size boundaries without rejected-file residue', async () => {
    ...
    const edgeTicket = await prisma.ticket.create({ data: { ticketNumber: 'TKT-20260901-16000004', requesterId: requesterAId, categoryId: template.categoryId, relatedSystemId: template.relatedSystemId, summary: 'Attachment edge case Ticket', description: 'Ticket used to verify edge cases in the Attachment API.', requestedPriority: 'LOW', itPriority: 'LOW' } });
    const edgeId = edgeTicket.id;
    const tempBefore = await directoryFiles(attachmentTempDirectory);
    const finalBefore = await directoryFiles(attachmentFinalDirectory);
    const invalidName = await request(app).post(`/api/tickets/${edgeId}/attachments`).set(await sessionHeaders(requesterAId)).attach('file', png, { filename: '.png', contentType: 'image/png' });
    expect(invalidName.status).toBe(400);
    expect(invalidName.body.error.code).toBe('ATTACHMENT_FILENAME_INVALID');
    const invalidExtension = await request(app).post(`/api/tickets/${edgeId}/attachments`).set(await sessionHeaders(requesterAId)).attach('file', png, { filename: 'malware.exe', contentType: 'image/png' });
    expect(invalidExtension.status).toBe(415);
    expect(invalidExtension.body.error.code).toBe('ATTACHMENT_TYPE_UNSUPPORTED');
    const mismatchedMime = await request(app).post(`/api/tickets/${edgeId}/attachments`).set(await sessionHeaders(requesterAId)).attach('file', png, { filename: 'mismatch.png', contentType: 'application/pdf' });
    expect(mismatchedMime.status).toBe(415);
    expect(mismatchedMime.body.error.code).toBe('ATTACHMENT_TYPE_UNSUPPORTED');
    const malformedSignature = await request(app).post(`/api/tickets/${edgeId}/attachments`).set(await sessionHeaders(requesterAId)).attach('file', Buffer.from('not a PNG'), { filename: 'malformed.png', contentType: 'image/png' });
    ...
    const oversized = await request(app).post(`/api/tickets/${edgeId}/attachments`).set(await sessionHeaders(requesterAId)).attach('file', large, { filename: 'large.png', contentType: 'image/png' });
    ...
    const acceptedBoundary = await request(app).post(`/api/tickets/${edgeId}/attachments`).set(await sessionHeaders(requesterAId)).attach('file', exactLimit, { filename: 'exact-limit.png', contentType: 'image/png' });
  ...
  it('covers removal validation/conflict, cross-requester byte/remove access, and no preview route', async () => {
    const template = await prisma.ticket.findFirstOrThrow({ where: { id: ticketId } });
    const edgeTicket = await prisma.ticket.create({ data: { ticketNumber: 'TKT-20260901-16000005', requesterId: requesterAId, categoryId: template.categoryId, relatedSystemId: template.relatedSystemId, summary: 'Attachment removal edge Ticket', description: 'Ticket used to verify removal edge cases.', requestedPriority: 'LOW', itPriority: 'LOW' } });
    const edgeId = edgeTicket.id;
    const active = await request(app).post(`/api/tickets/${edgeId}/attachments`).set(await sessionHeaders(requesterAId)).attach('file', png, { filename: 'remove-me.png', contentType: 'image/png' });
    expect(active.status).toBe(201);
    const id = active.body.data.id;
    expect((await request(app).delete(`/api/attachments/${id}`).set(await sessionHeaders(requesterAId)).send({ reason: 'bad' })).status).toBe(400);
    expect((await request(app).get(`/api/attachments/${id}/download`).set(await sessionHeaders(requesterBId))).status).toBe(404);
    expect((await request(app).delete(`/api/attachments/${id}`).set(await sessionHeaders(requesterBId)).send({ reason: 'A valid removal reason.' })).status).toBe(404);
    expect((await request(app).delete(`/api/attachments/${id}`).set(await sessionHeaders(requesterAId)).send({ reason: 'A valid removal reason.' })).status).toBe(200);
    expect((await request(app).delete(`/api/attachments/${id}`).set(await sessionHeaders(requesterAId)).send({ reason: 'A valid removal reason.' })).status).toBe(409);
    expect((await request(app).get(`/api/attachments/${id}/preview`).set(await sessionHeaders(requesterAId))).status).toBe(404);
  });
  ...
  it('compensates storage and metadata failures without deleting the Ticket or successful uploads', async () => {
    ...
    const storageFailure = await request(app).post(`/api/tickets/${ticketId}/attachments`).set(await sessionHeaders(requesterAId)).attach('file', png, { filename: 'storage-failure.png', contentType: 'image/png' });
    rename.mockRestore();
    ...
    const metadataFailure = await request(app).post(`/api/tickets/${ticketId}/attachments`).set(await sessionHeaders(requesterAId)).attach('file', png, { filename: 'metadata-failure.png', contentType: 'image/png' });
    log.mockRestore();
  ...
  it('returns a safe removal failure and leaves removal metadata unchanged', async () => {
    ...
    const response = await request(app).delete(`/api/attachments/${active.id}`).set(await sessionHeaders(requesterAId)).send({ reason: 'A valid removal reason.' });
    log.mockRestore();
  ...
  it('enforces the five-active limit, permits a replacement, and serializes concurrent admission', async () => {
    ...
    const sixth = await request(app).post(`/api/tickets/${ticketId}/attachments`).set(await sessionHeaders(requesterAId)).attach('file', png, { filename: 'sixth.png', contentType: 'image/png' });
    ...
    const replacement = await request(app).post(`/api/tickets/${ticketId}/attachments`).set(await sessionHeaders(requesterAId)).attach('file', png, { filename: 'replacement.png', contentType: 'image/png' });
    ...
    const [left, right] = await Promise.all([
      request(app).post(`/api/tickets/${concurrentTicketId}/attachments`).set(await sessionHeaders(requesterAId)).attach('file', png, { filename: 'parallel-left.png', contentType: 'image/png' }),
      request(app).post(`/api/tickets/${concurrentTicketId}/attachments`).set(await sessionHeaders(requesterAId)).attach('file', png, { filename: 'parallel-right.png', contentType: 'image/png' }),
    ]);
  ...
  it('returns a safe unavailable-file error without leaking private storage information', async () => {
    ...
    const response = await request(app).get(`/api/attachments/${unavailable.id}/download`).set(await sessionHeaders(requesterAId));
    expect(response.status).toBe(404);
