...
test('E2E-03 Admin creates, edits and resets an account; new credentials require password change', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Create account' }).click(); await expect(page.getByRole('status')).toContainText('Account saved successfully');
  ...
  await page.getByRole('button', { name: 'Set initial password', exact: true }).click(); await expect(page.getByRole('status')).toContainText('Initial password updated');
